...
const { Formula, EventRule, Org, Flare, Header, Job, 
  OrgInfo, VisibleEmission, Event, NumericEventRule,
  DailyNumericData, PiTag, FormulaValue,
} = require('./FRTModels');
...
class MongoData {
  ...
  async getOrgs(filter = {}) {
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          const orgs = await Org.find(filter).select('timezone dataRangeStart name').lean().exec()
          return resolve(orgs)
        }
  ...
  async getFormulaData(filter = {}) {
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          let data = await FormulaValue.find(filter).lean().exec()
          return resolve(data)
        }
        catch (error) { return reject(getErrorObject(error, "Mongo.getFormulas(): ")) }
      })();
