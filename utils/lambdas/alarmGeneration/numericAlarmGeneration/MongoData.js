...
const { User, Formula, EventRule, Org, Flare, Header, Job, OrgInfo, VisibleEmission, Event, NumericEventRule, DailyNumericData } = require('./FRTModels');
...
class MongoData {
  ...
  async bulkUpdateUser(bulkOps) {
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          let res = await User.bulkWrite(bulkOps);
          return resolve(res)
        }
        catch (error) { return reject(getErrorObject(error, "Mongo.bulkUpdateUser(): ")) }
      })();
    });
  }
