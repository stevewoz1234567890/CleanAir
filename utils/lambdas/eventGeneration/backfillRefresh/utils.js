...
const { Event, DebugEvent, EventRule, Org } = require('./FRTModels')
...
class Mongo {
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          return reject(`Error closing client`);
        }
      })();
    });
  }
   ...
   */
  async uploadEventsToMongo(eventArray) {
    return new Promise((resolve, reject) => {
      (async () => {
        try {
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          // console.log("eventArray: ", isDebug, util.inspect(eventArray, { depth: null }));
          // exit()
          if (eventArray.length < 1) return resolve("No events to upload");
          let ruleId = eventArray[0].eventRule;
  ...
  async removeEvents(filter) {
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          console.log("invoking delete many")
          let doc = await Event.deleteMany(filter);
