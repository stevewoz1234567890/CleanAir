    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          if (rule.resolution === 1 && rule.withValues && rule.checkFor && period.withNumValue === null) {
          ...
          else if (rule.resolution === 15 && rule.withValues && rule.checkFor) { // && period.withNumValue === null
            let collection = debug === true ? DebugFormulaValue : FormulaValue;
            let filter = {
              formula: rule.checkForValue,
            ...
            period.withNumValue = vals.map(v => {
              return { value: v.value, _id: v._id, date: v.date }
            })
            return resolve(period)
          }
          else {
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          return reject(getErrorObject(err, "WidgetMain.handleEdgeEvents()")) 
        }
      })();
    })
  }
