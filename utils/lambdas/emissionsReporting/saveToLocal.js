    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          if (formula.reportInfo[flare._id.toString()].firstValue.isCutoff) firstDate = formula.reportInfo[flare._id.toString()].firstValue.newStartObject.date;
          ...
          let pivotDate = DateTime.fromMillis(firstDate.toMillis(), { zone: this.timezone });
          ...
          yearMonthBins = yearMonthBins.map(v => {
            ...
            switch (this.action) {
              case 'sum':
                resultValue = binData.reduce(sumReducer).value;
                break;
              case 'average':
                let sum = binData.reduce(sumReducer).value;
                resultValue = sum / binData.length;
                break;
              default:
                throw new Error(`No matching action available to: ${this.action}`);
            }
            v.result = parseFloat(resultValue.toFixed(2));
            return v;
          });
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          } else rawData = await this.mongo.getFormulaValues(filter, !this.payload.debug);
          let filename = `${formula._id.toString()}_${flare._id.toString()}_${this.dashStart}_${this.dashEnd}.json`
          let path = LOCAL_DATA_PATH + filename;
          console.log({path})
          if (!USE_LOCAL_DATA) await fs.writeFile(path, JSON.stringify(rawData))
          return resolve()
