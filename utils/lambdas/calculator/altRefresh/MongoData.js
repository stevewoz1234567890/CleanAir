...
class MongoData {
  ...
  async getEventRules(orgOID) {
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          if (this.allData.eventRules) {
            return resolve(this.allData.eventRules)
          }
          const eventRules = await EventRule.find({ org: orgOID }).select('_id checkForValue name checkFor withValues formula resolution').lean().exec();
          let ruleMap = {};
          ...
          this.allData.eventRuleFormulaIDs = ruleFormulasIDs;
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          // console.log(dependency.attr.includes('previous 15'), dependency.type === 'parameter')
          ...
          if (dependency.type === 'parameter') {
            let matchingTag = this.allData.piTags.find(tag => {
              ...
              // console.log("flare match: ", typeof tag.flare._id.toString(),tag.flare._id.toString(), typeof flareID, flareID)
              ...
              return (paramMatch && flareMatch && headerMatch);
            });
            if (!matchingTag) return resolve(null);
            let matchingTagOID = matchingTag._id;

            // console.log("matchingTagOID: ", matchingTagOID);
            // return resolve(null);
 ...
 */
  async getFormulaArrayValues(dependancy, isDebug, endDate, orgOID, flareOID, headerID = null, timespan = 15, valuesOnly = true) {
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          if (valuesOnly){
            ...
            vals = result;
            vals.unshift(dependancy.fullValue);
            // while (vals.length < timespan) {
            //   vals.push(null);
            // }
          }
