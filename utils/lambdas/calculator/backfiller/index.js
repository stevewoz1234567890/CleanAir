  ...
  */
  ...
  async updateOrg(payload) {
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          let orgs = await this.mongo.getOrgs();
          let org = orgs.find(o=>o._id.toString()===payload.o);
          if (!org) return resolve();
          if (payload.t.length === 0) return resolve();
          let timestamps = payload.t.filter(t => !isNaN(t));
          let newestTimestamp = Math.max(...timestamps);
          let updateMade = false;
          for (let item of org.backfillQueue) {
            //We need to find the last 1-minute mark (non-15th) and the last 15th
            let min1, min15 = null;
            let incr = 0;
            let it = new Date(item.newestDate.getTime());
            while (it.getTime() >= item.oldestDate.getTime()) {
              if (min1 && min15) break;
              if (it.getMinutes() % 15 === 0) min15 = new Date(it.getTime());
              if (it.getMinutes()%15 !== 0) min1 = new Date(it.getTime());
              it = new Date(it.getTime() - 60000);
              incr++;
              if (incr > 17) {
                console.log("UNEXPECTED (not error): incrementor over 17");
                if (incr > 60) break;
              }
            }
            ...
            if (min1) {
              min1 = min1.getTime()
              let min1Match = timestamps.find(t=>t===min1)
              if (min1Match) item.isCalcsComplete1 = true;
              updateMade = true;
            }
            if (min15) {
              min15 = min15.getTime()
              let min15Match = timestamps.find(t=>t===min15)
              if (min15Match) item.isCalcsComplete15 = true;
              updateMade = true;
            }
            
            item.isCalcsCompleteLastCalc = new Date();
            updateMade = true;
            
            if (item.hasOwnProperty("newestDateFound")) {
              if (item.newestDateFound === null) item.newestDateFound = new Date(newestTimestamp);
              else if (item.newestDateFound.getTime() < newestTimestamp) item.newestDateFound = new Date(newestTimestamp);
              updateMade = true;
            } else {
              console.log("newestDateFound key NOT found")
              item.newestDateFound = new Date(newestTimestamp);
              updateMade = true;
            }
          }
          if (updateMade) {
            // console.log("updating the backfillQueue in DB: ", {_id:org._id}, org.backfillQueue)
            let res = await this.mongo.updateOrg({_id:org._id}, {backfillQueue : org.backfillQueue});
            // console.log("response to updateOrg: ", res)
          }
          return resolve();
        } catch (error) { return reject(getErrorObject(error, "LiveDataProcessor.updateOrg():")) }
      })();
    });
  }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    for (let record of event.Records) {
      ...
      await PROCESSOR.updateOrg(payload);
      console.log(`finished processing: ${payload.retDateString}`)
      if (DEBUG_1 || DEBUG_2) console.log(`Done Processing: ${processingDisplay}`);
      returnString += payload.retDateString;
      returnString += "\n";
    }
    if (LOCAL_ENV) {
    ...
    const response = {
      statusCode: 200,
      body: JSON.stringify({ "processedDate": returnString }),
    }
