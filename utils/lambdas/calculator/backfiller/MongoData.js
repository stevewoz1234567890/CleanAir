...
const { Formula, Flare, Header,
  ...
  CompoundGroup, PiValuesDebug, EventRule, Org
} = require('./FRTModels');
...
class MongoData {
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          orgs = orgs.filter(o=>o.hasOwnProperty("backfillQueue"));
          return resolve(orgs);
        }
        catch (error) {
          if (error.hasOwnProperty('printPath')) {
            let errObj = { printPath: "Mongo.getOrgs(): ", error: error.error };
            errObj.printPath = `${errObj.printPath}${error.printPath}`
            return reject(errObj);
          }
          return reject({ printPath: "Mongo.getOrgs(): ", error });
        }
      })();
    });
  }
  ...
  async updateOrg(filter, update) {
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          let doc = await Org.updateOne(filter, update);
          return resolve(doc)
        }
        catch (error) { return reject(getErrorObject(error, "Mongo.findAndUpdateOrg(): ")) }
      })();
    });
  }
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const eventRules = await EventRule.find({ org: orgOID }).select('_id checkForValue name checkFor withValues formula resolution use').lean().exec();
          let ruleMap = {};
