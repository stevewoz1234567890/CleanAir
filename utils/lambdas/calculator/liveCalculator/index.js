...
var SKIP_CHECKER_ON = true;
var SQS_DEBUG = false;
var DIRECT_CALC = true;
var UPLOAD_DEBUG = false;
var DEBUG_2 = false; //More logging
var DEBUG_1 = false; //Less logging
var LOCAL_ENV = false; //diffrent kinds of processing needed. Like parsing a json object or not.
var SAVE_VALUES = true; //save the values you calculate
var FORWARD_TO_SQS = true; //forward to sqs for event gen
var DUMP_RESULTS = false;
var FORWARD_NUM_RULE_SQS = false;
...
function setSettings(event) {
  if (event.hasOwnProperty("invokerArgs")) {
    ...
    let s = e.settings;
    DEBUG_TIMER = s.DEBUG_TIMER;
    SKIP_CHECKER_ON = s.SKIP_CHECKER_ON;
    SQS_DEBUG = s.SQS_DEBUG;
    DIRECT_CALC = s.DIRECT_CALC;
    UPLOAD_DEBUG = s.UPLOAD_DEBUG;
    DEBUG_2 = s.DEBUG_2;
    DEBUG_1 = s.DEBUG_1;
    LOCAL_ENV = s.LOCAL_ENV;
    SAVE_VALUES = s.SKIP_CHECKSAVE_VALUESER_ON;
    FORWARD_TO_SQS = s.FORWARD_TO_SQS;
    DUMP_RESULTS = s.DUMP_RESULTS;
    FORWARD_NUM_RULE_SQS = s.FORWARD_NUM_RULE_SQS;
    if (LOCAL_ENV) require('dotenv').config();
    return e.body;
  }
  return event;
}
...
class LiveDataProcessor {
                          ...
                          (async () => {
                            try {
                              ...
                              const schema = {
                                ...
                                numericEventRules: formula.numericEventRules ?? null,
                              }
                ...
                (async () => {
                  try {
                    ...
                    let isComplete = this.mongo.allData.piTags.every(tag => {
                      ...
                      if (!tagMatched) {
                        console.log("NOT MATCHED (will not calc on this timestamp): ", tag.name)
                      }
                      return tagMatched;
                    })
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          const bulkUpdates = [];
          const ruleUpdates = [];
          const numericRuleUpdates = [];
          let is15MinDate = date.getMinutes() % 15 === 0;
          await Promise.all(formulaObjects.map(async (formula) => {
            let isNumRuleRelated = false;
            let resolution = 1; //i'm confused... doesn't formula have it's own resolution attribute? NO!... This is a flaw...
            if (Array.isArray(formula.numericEventRules) && formula.numericEventRules.length > 0) {
              //Numeric Event Rules of a given parameter should be, by definition, all the same resolution...
              let rule = this.mongo.allData.numericEventRules.find(e => e._id.toString() === formula.numericEventRules[0].toString())
              resolution = rule.resolution;
              isNumRuleRelated = true;
            }
            else if (formula.ruleInfo) { //boolean event rule
            ...
            if (!isValidDate(dbSchema.start)) console.log("invalid date: ", {dbSchema, date, startDate, resolution})
            ...
            if (isEventRuleRelated) {
              ...
              //else if it is a withValue that should be sent with a rule
                ...
                if (!sentWithRule) bulkUpdates.push(dbSchema); //case: is part of a off-time rule (e.g. a 1-min formula also used by a 15min rule) but is itself a regular formula that should always send
              }
            }
            else if (isNumRuleRelated) {
              let ruleInfo = {
                numericEventRules: formula.numericEventRules ?? null,
                parameterType: "formula",
              }
              numericRuleUpdates.push({ updateInfo : dbSchema, ruleInfo });
            }
            else {
          ...
          return resolve({ bulkUpdates, ruleUpdates, numericRuleUpdates });
        }
        catch (error) {
          if (error.hasOwnProperty('printPath')) {
  ...
  */
          ...
          let bulkOps = await Promise.all(bulkData.map(datum => {
            return new Promise((resolve, reject) => {
              (async () => {
                ...
                catch (error) { return reject({ printPath: "LiveDataProcessor._bulkPrepare_()._bulkPrepare_.map: ", error }) }
              })()
            })
          }))
          let debugOps = [];
          let prodOps = [];
          bulkOps.forEach(op => { //why isn't this instead two filter statements? Or 3 if we need to capture undefines?
      ...
      (async () => {
        try {
            ...
            if (ruleData.length > 0) {
              ...
              for (let a of ruleData) {
                ...
                for (let e of a) {
                  ...
                  console.log(e.ruleInfo.eventRule.use)
                  //entry.ruleInfo.eventRule.use === "alarming"
                  // console.log("entry: ", e)
                  // console.log("withValues: ", e.ruleInfo.withValue)
                  numItems++;
                }
                console.log("END======")
              }
          ...
          for (const arr of ruleData) {
            ...
            var alarmParams = {
              // QueueUrl: process.env.ALARM_GEN_SQS_URL,
              QueueUrl: process.env.RULE_GEN_SQS_URL,
              Entries: []
            };
            for (const message of arr) {
              console.log("sending message..")
              let eventRuleUse = message.ruleInfo.eventRule.use;
              // console.log("sample id:" , uuidv4())
              if (eventRuleUse === "alarming") {
                alarmParams.Entries.push({
                  Id: uuidv4(),
                  MessageBody: JSON.stringify(message)
                });
              } else if (eventRuleUse === "reporting") {
                ruleParams.Entries.push({
                  Id: uuidv4(),
                  MessageBody: JSON.stringify(message)
                });
              }
            ...
            if (!LOCAL_ENV || FORWARD_TO_SQS) {
              if (alarmParams.Entries.length > 0) {
                await SQS.sendMessageBatch(alarmParams).promise();
              }
              if (ruleParams.Entries.length > 0) {
                await SQS.sendMessageBatch(ruleParams).promise();
              }
            }
          }
          return resolve();
          ...
          console.log("DUMP:")
          console.log(alarmParams, ruleParams)
          return reject(getErrorObject(error, "LiveDataProcessor._RuleDataToSQS_():"))
        }
      })();
    })
  }
  ...
  /**
 * Send data to SQS Queue selected by user. This is new and is intended as a more versatile way to
 * send data to SQS that can remove the need of _RuleDataToSQS_ but can be use by both bool and num rules.
 * Of course, num rules is a completely new item with unique behaviour...
 * @param {Array} ruleData This must be an array of arrays (max len of 10 per inner-array) 
 * @returns 
 */
  async _upload_to_sqs_(sqsData, sqsQueue, isFIFO=false) {
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          if (sqsData.length == 0) return resolve();
          let allEntries = sqsData.map(datum => {
            let formattedEntry = {
              Id : uuidv4(),
              MessageBody : JSON.stringify(datum),
            }
            if (isFIFO) {
              formattedEntry.MessageGroupId = uuidv4();
              formattedEntry.MessageDeduplicationId = uuidv4();
            }
            return formattedEntry;
          });
          let groupedEntries = chunkArray(allEntries, 10); //SQS has a max length of 10 messages per batch
          let messageBatches =  groupedEntries.map(entries => {
            return {
              QueueUrl: sqsQueue,
              Entries: entries
            }
          });
          for (const batch of messageBatches) await SQS.sendMessageBatch(batch).promise();
          return resolve();
        } catch (error) {
          return reject(getErrorObject(error, "LiveDataProcessor._upload_to_sqs_():"))
        }
      })();
    })
  }
  ...
  /**This function sends numerical rule data to a Lambda for processing as well as saves it to MongoDB */
  async _numericRuleDataUpload_(numericRuleData) {
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          //numericRuleData = [{updateInfo, ruleInfo}, ...]
          let databaseData = numericRuleData.map(d => d.updateInfo);
          if (FORWARD_NUM_RULE_SQS) {
            let res = await Promise.all([
              this._bulkPrepareAndUpload_(databaseData, {invoker:"_numericRuleDataUpload_"}),
              this._upload_to_sqs_(numericRuleData, process.env.NUM_RULE_FIFO_QUEUE, true)
            ]);
          } else {
            await Promise.all([
              this._bulkPrepareAndUpload_(databaseData, {invoker:"_numericRuleDataUpload_"}),
          ...
          return resolve();
        } catch (e) { return reject(getErrorObject(e, "LiveDataProcessor._numericRuleDataUpload_()")) }
      })();
    })
  }
  ...
  async _bulkPrepareAndUpload_(bulkData, options={}) {
    return new Promise((resolve, reject) => {
      (async () => {
        try {
          ...
          resolve();
        } catch (error) { return reject(getErrorObject(error, "LiveDataProcessor._bulkPrepareAndUpload_():")) }
      })();
    });
  }
      ...
      (async () => {
        try {
          ...
          for (let flareGroup of data) {
            ...
            numericRuleData = numericRuleData.concat(flareGroup.numericRuleUpdates);
          }
          if (FORWARD_NUM_RULE_SQS) var promises = [this._numericRuleDataUpload_(numericRuleData)];
          else var promises = [this._rulePrepareAndUpload_(ruleData), this._bulkPrepareAndUpload_(bulkData), this._numericRuleDataUpload_(numericRuleData)]
        ...
        try {
          ...
          try { //Temp fix to speed up live-only data that excludes the backlogs
            if (SKIP_CHECKER_ON) {
              ...
              {
                let numTags = this.mongo.allData.piTags.length;
                let pValColl = payload.d ? PiValuesDebug : PiValue
                let res = await pValColl.find({ date: new Date(payload.t[0]), org: new ObjectId(payload.o) }).select("-_id piTag value").lean().exec();
                if (numTags > res.length) {
                  return resolve()
                }
              }
          ...
          if (calcGroups.length === 0) return resolve();
          if (DEBUG_2) console.log("Num Calc Groups: ", calcGroups.length);
          if (this.mongo.allData == null) await this.mongo.getAllData();
          ...
          if (DUMP_RESULTS) console.log("data to upload: ", util.inspect(dataToUpload, { showHidden: false, depth: null, maxArrayLength: 3 }))
          if (!LOCAL_ENV || SAVE_VALUES || FORWARD_NUM_RULE_SQS) await this.uploadFormulaValues(dataToUpload)
          else if (DEBUG_1) {
